...
export class AdminService {
  ...
  async getStats(timeRange: 'week' | 'month' | 'year' = 'month') {
    ...
    const [totalServices, pendingServices, activeServices, rejectedServices] =
      await Promise.all([
        ...
        this.serviceModel.countDocuments({ status: ServiceStatus.ACTIVE }),
        this.serviceModel.countDocuments({ status: ServiceStatus.DISABLED }),
      ]);
  ...
  private async getUsersByMonth() {
    ...
    const toMap = (
      agg: { _id: { year: number; month: number }; count: number }[],
  ...
  private async getGovernorateStats() {
      ...
      this.userModel.aggregate([
        {
          $match: {
            role: 'client',
            'location.governorate': { $in: governorates },
          },
        },
    ...
    return bookingsAgg.map((b) => ({
      ...
      bookings: b.bookings,
      percentage:
        totalBookings > 0 ? Math.round((b.bookings / totalBookings) * 100) : 0,
    }));
  }
